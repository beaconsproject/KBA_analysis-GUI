ui = dashboardPage(skin="blue",
                   ...
                   dashboardSidebar(
                     ...
                     conditionalPanel(
                       ...
                       div(style = "margin-bottom: -20px;",fileInput(inputId = "upload_stream", label = "Upload streams dataset", multiple = TRUE)),
                       div(style = "margin-top: -20px;",fileInput(inputId = "upload_catch", label = "Upload catchments dataset", multiple = TRUE)),
                     ...
                     conditionalPanel(
                       ...
                       HTML("<h4>&nbsp; &nbsp; Set grid cell size</h4>"),
                       textInput("set_grid", "Set", value = 10000),
                       div(style = "margin: 15px; margin-top: -30px; font-size:13px;font-weight: bold", "Or "),
                       actionButton(inputId = "calc_dci", label = div(style = "font-size:13px;background-color:gey;color: black",HTML("Calculate DCI")), icon = icon(name = "check", lib = "font-awesome"), class = "btn-warning", style="width:250px"),
                       div(style = "margin-left: 20px;",downloadButton("download_shapefile2", "Download CAs with hydro attributes")),
                       tags$br(),
                       tags$br(),
                       tags$br(),
                       actionButton(inputId = "reduce_cas", label = div(style = "font-size:13px;background-color:gey;color: black",HTML("Reduce de number of conservation areas")), icon = icon(name = "check", lib = "font-awesome"), class = "btn-warning", style="width:250px"),
                       div(style = "margin-left: 20px;",downloadButton("download_shapefile3", "Download filtered CAs")),
                       tags$br(),
                       tags$br()
                     ), 
                     conditionalPanel(
